...
</div>`}function ie(e){const t=e.map(re).join("");s.feedbacksContainer.innerHTML=t}function ne(e){const t=Math.floor(e/1e3),a=Math.floor(t/60),r=t%60,i=r<10?`0${r}`:r;return`${a}:${i}`}function oe(e){const{strArtist:t,strArtistThumb:a,intFormedYear:r,intDiedYear:i,strGender:n,intMembers:c,strCountry:p,strBiographyEN:y,genres:T,albumsList:B}=e,C=$(T),N=B.map(P=>{const{strAlbum:I,tracks:x}=P,O=x.map(F=>{const{intDuration:j,movie:L,strTrack:H}=F;return`<tr class="artist-modal-album-row">
